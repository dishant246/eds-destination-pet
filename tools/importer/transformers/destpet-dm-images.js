 ...
 * Source uses Scene7 IS/Image URLs (host images.destpet.com, path /is/image/).
 * Rewrites DM <img> tags into anchors that round-trip through markdown; the
 * companion auto-block in scripts/scripts.js rebuilds them as responsive
 * <picture> elements at render time.
 *
 * Runs in afterTransform ONLY — block parsers run between the hooks and extract
 * <img> references into block cells (cards, columns-media). Rewriting imgs to
 * anchors earlier would empty those cells.
 *
 * Helpers below are copied verbatim from the canonical dm-scene7-helpers.js —
 * keep byte-identical, do not re-derive.
 */
